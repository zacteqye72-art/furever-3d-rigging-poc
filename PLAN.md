...
### 第 1 周：纯 API 端到端跑通（零自部署）

只需 `FAL_KEY` + `TRIPO_API_KEY`，不碰任何自部署。
...
- 跑 10 张测试照片通过完整 API 管线
- 记录：成功率、每步耗时、总成本、输出质量
- 初步评估四维度质量（骨架放置 / 蒙皮形变 / 末端处理 / 关节方向）
- **交付物：** 10 个带 `quadruped:walk` 动画的绑骨 GLB 文件 + 成本表 + 质量评估

### 第 2 周：根据第 1 周结果决定是否上 UniRig + 自定义动画

如果第 1 周 Tripo 质量达标 → 跳过 UniRig，直接进入动画优化：
- 编写 Blender 无头脚本：呼吸 + 眨眼 + 摇尾巴
- 对 Tripo 绑骨模型应用待机动画
- **交付物：** 10 个带情感待机动画的 GLB 文件

如果第 1 周 Tripo 质量不够 → 上 UniRig 做对比：
- 在 RunPod 上部署 UniRig（A10G 或 L4 GPU）
- 实现 `stage3b_rig_unirig.py` 封装三步 CLI
- 用同样 10 张照片跑 UniRig 路径
- 编写 Blender 无头脚本 + 应用待机动画
- **交付物：** 两条路径各 10 个带待机动画的 GLB 文件 + 质量对比
